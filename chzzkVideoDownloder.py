...
class ChzzkDownloaderV8:
    def __init__(self, root):
        self.root = root
        self.root.title("치지직 다운로더 (최종 수정: 쿠키 파일 & 단일 스트림)")
        ...
        self.items_data = {}
        self.temp_cookie_file = None # 임시 쿠키 파일 경로 저장
    ...
    def create_widgets(self):
        ...
        tk.Label(top_frame, text="치지직 다운로더 v8", font=("Bold", 14)).pack(side="left")
    ...
    def create_cookie_file(self):
        ...
        try:
            ...
            with os.fdopen(fd, 'w') as f:
                ...
                f.write("# This file is generated by Chzzk Downloader\n")
                # 도메인, 포함여부, 경로, 보안, 만료시간, 이름, 값
                # 만료시간은 대략 길게 설정 (2030년)
                expire = "1900000000"
    ...
    def get_ydl_opts(self, out_tmpl):
        ...
        opts = {
            ...
            'no_warnings': True,
            # [핵심 수정] 화질 선택 순서 변경
            # best: 단일 파일 최고화질 (에러 확률 낮음)
            # bestvideo+bestaudio: 분할 파일 합치기 (에러 확률 높음)
            'format': 'best/bestvideo+bestaudio',

            # Initialization fragment 에러 방지용 옵션 유지
            'hls_use_mpegts': True,
        }

        # 2. 쿠키 파일 적용 (헤더 방식 대신 사용 -> 경고 해결)
        if cookie_path:
